...
Eco_priorities <- read.csv("ECO priorities.csv")

mature_high <- Eco_priorities$Mature.Forest_High.Priority
mature_high <- mature_high[mature_high != ""]
post_fire_high <- Eco_priorities$Fire_High.Priority
post_fire_high <- post_fire_high[post_fire_high != ""]
post_harvest_high <- Eco_priorities$Harvest_High.Priority
post_harvest_high <- post_harvest_high[post_harvest_high != ""]

mature_lower <- Eco_priorities$Mature.Forest_Lower.Priority
mature_lower <- mature_lower[mature_lower != ""]
post_fire_lower <- Eco_priorities$Fire_Lower.Priority
post_fire_lower <- post_fire_lower[post_fire_lower != ""]
post_harvest_lower <- Eco_priorities$Harvest_Lower.Priority
post_harvest_lower <- post_harvest_lower[post_harvest_lower != ""]
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    that are selected with a checkbox, click the Reset BEU/BEC button. You can also
      ...
      fluidRow(
        # Column 1: Unharvested
        column(width = 6,
               HTML("<strong>Mature - High Priority</strong>"),
               checkboxGroupInput("checkMatureHigh", label = NULL,
                                  choices = setNames(mature_high,mature_high),
                                  selected = mature_high,
                                  inline = TRUE)
        ),

        # Column 2: Post-fire
        column(width = 6,
               HTML("<strong>Post-fire - High Priority</strong>"),
               checkboxGroupInput("checkPostFireHigh", label = NULL,
                                  choices = setNames(post_fire_high,post_fire_high),
                                  selected = post_fire_high,
                                  inline = TRUE)
        ),

        # Column 3: Post-harvest
        column(width = 6,
               HTML("<strong>Post-harvest - High Priority</strong>"),
               checkboxGroupInput("checkPostHarvestHigh", label = NULL,
                                  choices = setNames(post_harvest_high,post_harvest_high),
                                  selected = post_harvest_high,
                                  inline = TRUE)
        ),
        column(width = 6,
               HTML("<strong>Mature - Lower Priority</strong>"),
               checkboxGroupInput("checkMatureLow", label = NULL,
                                  choices = setNames(mature_lower,mature_lower),
                                  selected = mature_lower,
                                  inline = TRUE)
        ),

        # Column 2: Post-fire
        column(width = 6,
               HTML("<strong>Post-fire - Lower Priority</strong>"),
               checkboxGroupInput("checkPostFireLow", label = NULL,
                                  choices = setNames(post_fire_lower,post_fire_lower),
                                  selected = post_fire_lower,
                                  inline = TRUE)
        ),

        # Column 3: Post-harvest
        column(width = 6,
               HTML("<strong>Post-harvest - Lower Priority</strong>"),
               checkboxGroupInput("checkPostHarvestLow", label = NULL,
                                  choices = setNames(post_harvest_lower,post_harvest_lower),
                                  selected = post_harvest_lower,
                                  inline = TRUE)
        )
      ),
...
server <- function(input, output, session) {
  ...
  get_all_selected_ecotypes <- reactive({
    unique(c(
      input$checkMatureHigh,
      input$checkPostFireHigh,
      input$checkPostHarvestHigh,
      input$checkMatureLow,
      input$checkPostFireLow,
      input$checkPostHarvestLow
    ))
  })

  observeEvent(input$btnRevealECO,{

    current_eco <- get_all_selected_ecotypes()

    new_ecos <- unique(c(current_eco, ECOvec_store$data, input$Eco_text))
    new_ecos <- new_ecos[new_ecos != ""]

    if (length(new_ecos) > 0) {
      ECOvec_store$data <- new_ecos
    } else {
      showNotification("Please select at least one valid Ecotype or enter one manually.",
                       type = "error")
    }

  })
  observeEvent(input$btnClearECO, {
    updateCheckboxGroupInput(session, "checkMatureHigh", selected = character(0))
    updateCheckboxGroupInput(session, "checkPostFireHigh", selected = character(0))
    updateCheckboxGroupInput(session, "checkPostHarvestHigh", selected = character(0))
    updateCheckboxGroupInput(session, "checkMatureLow", selected = character(0))
    updateCheckboxGroupInput(session, "checkPostFireLow", selected = character(0))
    updateCheckboxGroupInput(session, "checkPostHarvestLow", selected = character(0))
  ...
  observeEvent(input$btnResetECO, {
    updateCheckboxGroupInput(session, "checkMatureHigh", selected = mature_high)
    updateCheckboxGroupInput(session, "checkPostFireHigh", selected = post_fire_high)
    updateCheckboxGroupInput(session, "checkPostHarvestHigh", selected = post_harvest_high)
    updateCheckboxGroupInput(session, "checkMatureLow", selected = mature_lower)
    updateCheckboxGroupInput(session, "checkPostFireLow", selected = post_fire_lower)
    updateCheckboxGroupInput(session, "checkPostHarvestLow", selected = post_harvest_lower)

    # Also update the reactiveValues store
    ECOvec_store$data <- unique(c(
      mature_high, post_fire_high, post_harvest_high,
      mature_lower, post_fire_lower, post_harvest_lower
    ))
  })
